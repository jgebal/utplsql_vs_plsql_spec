CREATE OR REPLACE PACKAGE BODY ut_message_api_with_rec IS
  ...
  PROCEDURE ut_send_receive_msg IS
    ...
    BEGIN
      ...
      utAssert.eq ( 'Then the message contains the same number', l_msg_sent.msg_number, l_msg_received.msg_number);
      utAssert.eq ( ' and the message contains the same date',   l_msg_sent.msg_text  , l_msg_received.msg_text  );
      utAssert.eq ( ' and the message contains the same text',   l_msg_sent.msg_date  , l_msg_received.msg_date  );
    END;
